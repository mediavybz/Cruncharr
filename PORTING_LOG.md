...
## Last Updated: 2026-07-23 (Round 47 recursive full-stack audit complete; 1.0.63 release verified)

---

## Round 47 — Recursive Full-Stack Audit and Stable Promotion (2026-07-23)

The recursive audit covers every tracked backend, frontend, test, build, container, template, and
release file. The first backend trust-boundary pass found that webhook URLs were validated before
send, but the default HTTP transport could follow a public redirect to a private address and could
resolve a different address when it opened the socket. The first pass repaired the actionable
trust-boundary, state-consistency, persistence, concurrency, lifecycle, and frontend-encoding
findings listed below. A second complete pass found and repaired the remaining auth profile-switch
serialization and explicit-empty filename-setting cases. The third complete pass found no
actionable defects. The release gate then found three whitespace-only formatter residues; after
normalizing those blocks, the repeated audit and every configured verification gate returned clean.
...
|------|-------------|---------|------|
| src/Cruncharr.Core/Utils/WebhookUrlValidator.cs | Existing Docker/web webhook SSRF adapter | [PT] Adds the shared hardened webhook handler: redirects are disabled, non-public special-use IPv4/IPv6 ranges are rejected, and DNS answers are revalidated and pinned when the socket connects, closing redirect and DNS-rebinding bypasses without changing notification payloads | 2026-07-23 |
| src/Cruncharr.API/Program.cs | Existing Docker/web HTTP-client wiring | [PT] Registers the dedicated `CruncharrWebhooks` client with a 30-second timeout and the shared hardened handler; existing image and general clients are unchanged | 2026-07-23 |
| src/Cruncharr.Core/Services/NotificationService.cs | Existing desktop notification event/payload port | [PT] Sends unchanged events and bodies through the named hardened transport; serializes login/update deduplication and records delivery only after success so transient failures retry without concurrent duplicates | 2026-07-23 |
| src/Cruncharr.API/Controllers/ConfigController.cs | Existing web settings adapter for desktop notification/Sonarr/proxy/stream/queue/download configuration | [PT] Sends the unchanged webhook test payload through the same named hardened transport used by saved notifications; preserves stored Sonarr/proxy and custom stream-endpoint secrets when sanitized settings are posted back; applies reset queue/transcode defaults to the running queue service; distinguishes omitted filename fields from explicit empty values so the existing “leave empty” template and whitespace controls can be cleared; and applies SDK-standard indentation to the unchanged configuration DTO initializer; routes, response shapes, and status behavior are unchanged | 2026-07-23 |
| src/Cruncharr.Core.Tests/SecurityRegressionTests.cs | Regression guards for Docker/web trust boundaries and runtime settings | [PT] Guards the no-redirect/connection-validation transport, an actual connection-time localhost rejection, non-public special-use IP ranges, NotificationService hardened-client use and failure retry/dedup behavior, cross-domain cookie suppression, serialized cancellation-aware auth refresh/login/logout/profile-switch state, concurrent and relative-path configuration saves, explicit clearing of clearable filename settings, constrained legacy queue-file ID matching, preservation of stored Sonarr/proxy/stream secrets, live queue-limit reset, and disabled update-checker lifetime | 2026-07-23 |
| src/Cruncharr.API/Services/AutoDownloadSchedulerService.cs | Existing desktop history refresh/auto-add scheduling exposed as a Docker hosted service | [PT] Serializes timed and manual checks through one gate so overlapping triggers cannot duplicate the same missing episode and locks the non-atomic nullable last-run timestamp shared with the status endpoint; eligibility, refresh, and queue-add logic remain unchanged | 2026-07-23 |
| src/Cruncharr.API/Controllers/SchedulerController.cs | Existing web action for the desktop manual refresh | [PT] Passes the HTTP request-abort token into the unchanged scheduler check; route, response shape, and success status remain unchanged | 2026-07-23 |
| src/Cruncharr.Core.Tests/QueuePumpEligibilityTests.cs | Existing desktop queue/scheduler and Docker/web broadcast behavior guards | [PT] Adds concurrency regressions proving overlapping scheduler calls execute one at a time with coherent state and slow SSE clients receive the latest complete queue snapshot instead of an obsolete buffered state | 2026-07-23 |
| src/Cruncharr.API/Services/UpdateCheckerService.cs | Existing desktop update-availability notification exposed as a Docker hosted service | [PT] Keeps the service alive while update notifications are disabled and re-evaluates the mutable setting each loop, so runtime enable/disable changes take effect without a container restart; release lookup, semver comparison, and notification behavior are unchanged | 2026-07-23 |
| src/Cruncharr.Core.Tests/SecurityRegressionTests.cs | Existing Docker/web trust-boundary and lifecycle guards | [superseded by consolidated Round 47 regression-suite row above] The disabled update-checker lifetime guard remains part of the suite | 2026-07-23 |
| src/Cruncharr.Core/Utils/HttpClientWrapper.cs | Existing desktop HTTP/cookie adapter shared by Crunchyroll and AniList calls | [PT] Filters the custom cookie store by the destination hostname before attaching values, preventing Crunchyroll session cookies from crossing into AniList or another external domain while preserving matching parent-domain behavior | 2026-07-23 |
| src/Cruncharr.Core/Services/LanguagePrefsService.cs | Docker/web adaptive-language state mirroring desktop language defaults | [PT] Returns a deep state snapshot while holding the existing lock so API serialization cannot race queue-driven learning and callers cannot mutate unpersisted internal collections; learning and persistence behavior are unchanged | 2026-07-23 |
| src/Cruncharr.Core.Tests/LanguagePrefsServiceTests.cs | Existing adaptive-language behavior guards | [PT] Adds a regression proving returned state is a deep snapshot and mutations do not alter the service | 2026-07-23 |
| src/Cruncharr.Core/Services/HistoryService.cs | Existing desktop rich-history tree and Docker persistence adapter | [PT] Returns a detached rich-history tree using the existing source-generated persistence serializer while holding the history gate, preventing REST/calendar/scheduler reads from racing nested refresh mutations; schema and history logic are unchanged | 2026-07-23 |
| src/Cruncharr.Core.Tests/HistoryDownloadRecordTests.cs | Existing completed-download/rich-history behavior guards | [PT] Adds a regression proving nested mutations to a returned history snapshot cannot alter service state and updates the partial-download setup to add newly available media through the service instead of mutating a leaked live episode reference | 2026-07-23 |
| src/Cruncharr.API/Services/QueueBroadcastService.cs | Docker/web real-time mirror of desktop queue-state refresh | [PT] Changes each bounded client channel to one latest complete snapshot with `DropOldest`, so a slow browser converges to final queue state rather than dropping the newest terminal event; SSE JSON and route are unchanged | 2026-07-23 |
| src/Cruncharr.API/Controllers/QueueController.cs | Existing REST file adapter over desktop DownloadHistory output records | [PT] Restricts the legacy prefix fallback to suffixes derived from supported Crunchyroll locales, preventing a shorter queue episode ID from resolving an unrelated longer history ID while retaining exact and legacy `JAJP`-style matches; route and valid-file behavior are unchanged | 2026-07-23 |
| src/Cruncharr.Core/Services/CrunchyrollAuthService.cs | Existing desktop Crunchyroll auth state used concurrently by Docker/web status polling, login/logout, profile switching, and downloads | [PT] Serializes unchanged refresh, login, logout, and profile-switch flows through one gate so callers cannot race token rotation, token persistence, profile selection, or logout; refresh/login/profile waits remain cancellation-aware | 2026-07-23 |
| src/Cruncharr.Core/Configuration/CruncharrConfig.cs | Existing desktop configuration model and Docker/web persistence adapter | [PT] Serializes the existing atomic save operation inside the owning model so settings, adaptive-language, and profile-sync callers cannot collide on the shared sibling temp file, and retains valid working-directory behavior when the configured path has no directory component; schema and serialization formats are unchanged | 2026-07-23 |
| src/Cruncharr.Core/Services/EncodingService.cs | Existing desktop custom encoding-preset persistence | [PT] Performs add/remove persistence and runtime-list mutation under the existing service lock, committing memory only after the corresponding disk operation succeeds and cleaning failed add temp files; preset schema and encoding behavior are unchanged | 2026-07-23 |
| src/Cruncharr.Core.Tests/EncodingPresetAndTranscodeTests.cs | Existing desktop preset and Docker transcode guards | [PT] Adds a regression proving a failed custom-preset disk commit does not activate an unpersisted preset in runtime state | 2026-07-23 |
| src/Cruncharr.Core/Services/QueuePersistenceService.cs | Existing desktop queue persistence adapted to the Docker `/config` volume | [PT] Serializes all existing immediate save/delete operations through the service's existing lock so concurrent REST and download-state changes cannot collide on `queue.json.tmp`; snapshot schema and restore behavior are unchanged | 2026-07-23 |
| src/Cruncharr.Core/Services/CalendarService.cs | Existing desktop/custom Calendar source aggregation | [PT] Applies SDK-standard indentation to the already-audited AniList cache refresh critical section; executable statements, branching, API behavior, and data shapes are unchanged | 2026-07-23 |
| src/Cruncharr.Core/Services/DownloadService.cs | Existing desktop download/transcode pipeline | [PT] Applies SDK-standard multiline object-initializer formatting to the transcode-slot progress update; values, sequencing, cancellation, and download behavior are unchanged | 2026-07-23 |

### Frontend (Mode B)
| File | Desktop Equivalent | API Endpoints Used | Change | Date |
|------|-------------------|-------------------|--------|------|
| src/Cruncharr.API/wwwroot/js/app.js | Seasonal/Browse selectable series cards, authenticated web API transport, desktop encoding-preset setting, mark-watched action, and queue controls | Existing `/api/v1/*` routes only | Restricts API-key attachment to same-origin `/api/` URLs, encodes every dynamic series/music/episode/queue route ID, emits one combined class attribute so selectable Seasonal cards retain the clickable state, and escapes the persisted encoding-preset label before rendering it as HTML | 2026-07-23 |

### Release Metadata
| File | Change | Date |
|------|--------|------|
| src/Cruncharr.API/Cruncharr.API.csproj | [PT] Release assembly/file/package version 1.0.62 → 1.0.63; target framework and dependencies unchanged | 2026-07-23 |
| src/Cruncharr.API/Controllers/HealthController.cs | [PT] Unreachable informational-version fallback 1.0.62 → 1.0.63; route, response shape, status codes, and health logic unchanged | 2026-07-23 |
| src/Cruncharr.API/wwwroot/index.html | Existing web release asset refresh: aligned CSS and JavaScript cache keys 1.0.62 → 1.0.63 so clients receive the recursively audited frontend | 2026-07-23 |

### In Progress
| File | Mode | Blocker |
|------|------|---------|
| Recursive audit passes 1–3 | A/B | [completed] Pass 3 and the post-format repeat found no actionable defects; no open decision or blocker |

### Verification
- Baseline warning-as-error Release build passed with 0 warnings/errors; baseline full Release suite passed 208/208.
- Configured analyzers and analyzer/style verification passed; an opt-in `latest-all` audit reported advisory-only CLI rules (logging delegates, async console calls, `ConfigureAwait`, broad boundary catches, and one private-type sealing suggestion) outside the repository warning policy; NuGet reported no known vulnerable or deprecated direct/transitive packages.
- Frontend syntax, Compose rendering, XML parsing, shell syntax, and Git whitespace passed.
- The final sequential clean warning-as-error Release build passed with 0 warnings/errors and the complete suite passed 236/236 tests after all repairs and formatter normalization.
- SDK whitespace, configured analyzer/style, frontend JavaScript, Compose, Git Bash shell, XML, and Git diff checks returned clean; NuGet again reported no known vulnerable or deprecated direct/transitive packages.
- Cache-only production builds passed independently for `linux/amd64` and `linux/arm64`.
- The loaded `linux/amd64` production image became healthy at `1.0.63+local-audit-1.0.63`, served aligned 1.0.63 CSS/JavaScript keys, retained the CLI and pinned FFmpeg `N-125649-g8d394252d8-20260717`, ran PID 1 as UID 1000, and stopped gracefully with exit code 0.
- Browser verification against that container rendered Calendar and Browse, exposed released Calendar thumbnails as accessible Add Download actions, showed the expected isolated-account no-episodes state when an authenticated series list was unavailable, and recorded no console warnings or errors.
- Docker Scout was installed but registry vulnerability scanning remained unavailable without a Docker Hub login; this external scanner limitation did not affect the clean local dependency, compile, test, container-build, or runtime gates.
...
| Existing desktop webhook notifications exposed through Docker/web configuration | src/Cruncharr.Core/Utils/WebhookUrlValidator.cs | Webhooks use a dedicated transport that never follows redirects and revalidates every DNS answer at socket-connect time before pinning the connection to an allowed public address. Keep both preflight URL validation and connection-time validation. |
| Docker/web dependency-injection transport wiring | src/Cruncharr.API/Program.cs | `CruncharrWebhooks` is the sole transport for configured webhook delivery and webhook tests; retain the 30-second timeout and `WebhookUrlValidator.CreateHttpMessageHandler` registration. |
| Existing desktop notification dispatcher | src/Cruncharr.Core/Services/NotificationService.cs | Notification flags, event payloads, templates, and headers remain desktop-equivalent; HTTP delivery must use `CruncharrWebhooks`. Login/update deduplication is serialized and becomes final only after a successful send so transient failures remain retryable. |
| Desktop notification/Sonarr/proxy/stream/queue/download settings and test actions | src/Cruncharr.API/Controllers/ConfigController.cs | The webhook-test endpoint and saved notification delivery must share `CruncharrWebhooks`; keep the existing request/response contract and sanitized projection. Preserve stored webhook-header, Sonarr-key, proxy-password, and custom stream-endpoint authorization values when a sanitized response is posted back. Reset must apply default processing/transcode limits to the running queue service. Treat `null` as omitted but `""` as an explicit clear for the advanced filename template and filename whitespace substitute, matching their web controls. Round 47 also normalized only the DTO initializer indentation. |
| Docker/web security and runtime-settings regression suite | src/Cruncharr.Core.Tests/SecurityRegressionTests.cs | Preserve guards for image redirect behavior; webhook redirect, DNS-connect, special-use-address, and named-client protections; masked-secret round trips; live reset limits; and disabled update-checker lifetime. |
| Desktop HTTP/cookie adapter reused by Docker services | src/Cruncharr.Core/Utils/HttpClientWrapper.cs | The custom cookie store is shared by Crunchyroll calendar and AniList requests; `AttachCookies` must domain-match each stored cookie before adding it so Crunchyroll session state never crosses to an unrelated host. |
| Docker/web adaptive-language state | src/Cruncharr.Core/Services/LanguagePrefsService.cs | `State` is serialized by the REST controller while queue actions can record picks; return deep snapshots under `_lock` and never expose `_state` collections directly. |
| Adaptive-language regression suite | src/Cruncharr.Core.Tests/LanguagePrefsServiceTests.cs | Preserve the deep-snapshot guard alongside opt-in, threshold, decline, snooze, reset, and audio/sub independence behavior. |
| Desktop rich-history tree exposed to REST, Calendar, and Scheduler | src/Cruncharr.Core/Services/HistoryService.cs | `GetHistorySeriesAsync` must return a detached tree created under `_lock`; preserve the source-generated `HistoryJsonContext` schema so readers cannot race or mutate live nested collections. |
| Completed-download/rich-history regression suite | src/Cruncharr.Core.Tests/HistoryDownloadRecordTests.cs | Preserve the detached rich-history snapshot guard with completion marking, partial-download tracking, language casing, and poster refresh behavior. |
| Desktop history auto-refresh/auto-add scheduler | src/Cruncharr.API/Services/AutoDownloadSchedulerService.cs | The Docker timer and manual REST trigger share `_runGate`; keep the desktop eligibility and language precedence inside that gate so overlapping checks cannot enqueue duplicate queue IDs for one episode. |
| Desktop manual history refresh action | src/Cruncharr.API/Controllers/SchedulerController.cs | The REST trigger delegates to the shared scheduler gate and propagates `HttpContext.RequestAborted`; it must not duplicate scheduling logic in the controller. |
| Desktop queue/scheduler and Docker/web SSE regression suite | src/Cruncharr.Core.Tests/QueuePumpEligibilityTests.cs | Preserve the overlapping-check serialization and latest-SSE-snapshot guards alongside queue eligibility, pause/cancel, retry-wake, persistence, and replacement tests. |
| Desktop update-availability notification | src/Cruncharr.API/Services/UpdateCheckerService.cs | The Docker hosted service must remain alive when `NotifyUpdateAvailable` is false and re-read the mutable setting in its loop; disabled mode performs no GitHub request and polls locally once per minute so Settings changes do not require restart. |
| Docker/web security and lifecycle regression suite | src/Cruncharr.Core.Tests/SecurityRegressionTests.cs | [superseded by the consolidated security/runtime-settings note above] Preserve the disabled-update-checker lifetime guard with the existing trust-boundary tests. |
| Desktop queue-state refresh mirrored over Docker/web SSE | src/Cruncharr.API/Services/QueueBroadcastService.cs | Each event contains the complete queue state, so client channels must retain only the newest snapshot (`capacity=1`, `DropOldest`); buffering obsolete progress or dropping a terminal snapshot can leave the UI stale. |
| DownloadHistory.OutputPath REST adapter | src/Cruncharr.API/Controllers/QueueController.cs | File resolution first requires an exact episode ID; the legacy concatenated-audio fallback may match only suffixes derived from `Languages.languages`, never an arbitrary prefix. Continue canonical output-root confinement for the resulting path. |
| Desktop Crunchyroll auth state reused by Docker/web callers | src/Cruncharr.Core/Services/CrunchyrollAuthService.cs | Auth status polling, parallel downloads, and user login/logout/profile-switch actions share `_refreshTokenGate`; keep public refresh/login/profile waits cancellation-aware and run each original token-mutating flow only after the gate is acquired so token/profile state and persistence cannot race. |
| Desktop configuration persistence reused by multiple Docker/web API surfaces | src/Cruncharr.Core/Configuration/CruncharrConfig.cs | Keep the atomic sibling-temp write protected by the model's shared `_saveLock`; controller-local locks do not coordinate with adaptive-language or profile-sync saves to the same configuration path. |
| Desktop custom encoding-preset persistence | src/Cruncharr.Core/Services/EncodingService.cs | Keep disk commit and `_custom` mutation inside the same `_lock`, with disk first: a REST success/failure must leave the runtime preset list and persisted files in the same state. |
| Desktop queue persistence on the Docker configuration volume | src/Cruncharr.Core/Services/QueuePersistenceService.cs | Every save/delete must remain inside `_syncLock`; queue state changes arrive concurrently from REST actions, the pump, progress callbacks, and retry scheduling but share one `queue.json.tmp`. |
| Desktop/custom Calendar aggregation and AniList refresh | src/Cruncharr.Core/Services/CalendarService.cs | The AniList cache refresh remains serialized by `_anilistLoadLock`; Round 47 only normalized indentation inside that existing critical section and changed no executable behavior. |
| Desktop download/transcode pipeline | src/Cruncharr.Core/Services/DownloadService.cs | The transcode-slot progress notification remains immediately before the cancellation-aware slot wait; Round 47 only normalized its object-initializer layout. |
