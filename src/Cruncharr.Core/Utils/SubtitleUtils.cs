...
// [PT] Ported from upstream CRD/Downloader/Crunchyroll/Utils/SubtitleUtils.cs.
// Cleans Crunchyroll ASS, fixes Closed Caption Converter ("CCC") output, and
// upserts [Script Info] lines (PlayRes/ScaledBorderAndShadow). The upstream
// CrDownloadOptions/LanguageItem args are flattened to primitives here.
public static class SubtitleUtils
{
    ...
    // scaledBorder: config string "ScaledBorderAndShadowYes" / "ScaledBorderAndShadowNo" / "DontAdd".
    public static string CleanAssAndEnsureScriptInfo(string assText, bool fixCccSubtitles, string? scaledBorder, string crLocale)
    {
        if (string.IsNullOrEmpty(assText))
            return assText;

        string? scaledLine = NormalizeScaledBorder(scaledBorder);

        bool isCcc = assText.Contains("www.closedcaptionconverter.com", StringComparison.OrdinalIgnoreCase);

        if (isCcc && fixCccSubtitles)
        {
            ...
            assText = FixStyles(assText, crLocale);
        }
        ...
        // Collapse runs of blank lines (keep one between sections)
    ...
    public static string? NormalizeScaledBorder(string? scaledBorder)
    {
        if (string.IsNullOrEmpty(scaledBorder))
            return null;
        if (scaledBorder.Contains("Yes", StringComparison.OrdinalIgnoreCase) || scaledBorder.Equals("yes", StringComparison.OrdinalIgnoreCase))
            return "ScaledBorderAndShadow: yes";
        if (scaledBorder.Contains("No", StringComparison.OrdinalIgnoreCase) || scaledBorder.Equals("no", StringComparison.OrdinalIgnoreCase))
            return "ScaledBorderAndShadow: no";
        return null; // "DontAdd" / anything else
    }
